      ...
      <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;">
        ...
        <div style="background:#f8f9fa;padding:24px;border-radius:0 0 8px 8px;border:1px solid #e0e0e0;">
          <table style="width:100%;border-collapse:collapse;">
            <tr><td style="padding:8px 0;color:#666;width:140px;"><strong>Name</strong></td><td style="padding:8px 0;color:#222;">${displayName||"–"}</td></tr>
            <tr><td style="padding:8px 0;color:#666;"><strong>Email</strong></td><td style="padding:8px 0;color:#222;">${email||"–"}</td></tr>
            <tr><td style="padding:8px 0;color:#666;"><strong>Firm</strong></td><td style="padding:8px 0;color:#222;">${firmName||"–"}</td></tr>
            <tr><td style="padding:8px 0;color:#666;"><strong>BD Affiliation</strong></td><td style="padding:8px 0;color:#222;">${bdAffiliation||"–"}</td></tr>
            <tr><td style="padding:8px 0;color:#666;"><strong>Phone</strong></td><td style="padding:8px 0;color:#222;">${phone||"–"}</td></tr>
          </table>
          <div style="margin-top:24px;text-align:center;">
            <a href="${ADMIN_URL}" style="background:#c9a84c;color:#1a2035;padding:12px 32px;border-radius:6px;text-decoration:none;font-weight:bold;font-size:15px;display:inline-block;">Review in Admin Panel →</a>
          </div>
          <p style="color:#999;font-size:12px;margin-top:20px;text-align:center;">Alts Fund Link · For authorized use only</p>
        </div>
      </div>`,
  };
  try {
...
exports.sendStatusEmail = onRequest({ cors: true }, async (req, res) => {
  if (req.method !== "POST") { res.status(405).send("Method Not Allowed"); return; }
  const { to, subject, html } = req.body;
  if (!to || !subject || !html) { res.status(400).send("Missing fields"); return; }
  try {
    await transporter.sendMail({ from: `"Alts Fund Link" <${GMAIL_USER}>`, to, subject, html });
