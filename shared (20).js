...
(function (global) {
  ...
  const NUM_FIELDS = new Set([
    'y1coc','ltv','preferred',
    'holdPeriod','numAssets','occupancy','dscr','sqft','avgLeaseTerm',
    'capRate','repComp','salesLoad',
  ...
  function derivePctRemaining(fund) {
    const filed = parseMoneyStr(String(fund.filedRaise || ''));
    const remaining = parseMoneyStr(String(fund.equityRemaining || ''));
    if (!filed || !remaining) return null;
    const pct = (remaining / filed) * 100;
    return Math.min(100, Math.max(0, pct));
  }

  function deriveRaiseVelocity(fund) {
    const raised = parseMoneyStr(String(fund.currentRaise || ''));
    if (!raised || !fund.offeringOpen) return null;
    const open = parseGvizDate(fund.offeringOpen);
    if (!open) return null;
    const months = (new Date() - open) / (1000*60*60*24*30.44);
    if (months < 0.1) return null;
    return raised / months; // $/month
  }
  ...
  // ─────────────────────────────────────────────────────────────
  //  FIRESTORE CONFIG (mirrors auth-gate.js)
  // ─────────────────────────────────────────────────────────────
  ...
  // ─────────────────────────────────────────────────────────────
  //  FIRESTORE OFFERING LOADER
  //  Maps Firestore offering doc structure → shared.js fund shape
  // ─────────────────────────────────────────────────────────────
  async function loadFundsFromFirestore(userGroup) {
    try {
      ...
      // Reuse existing app or init
      const app = getApps().length ? getApps()[0] : initializeApp(FIREBASE_CONFIG, 'shared-loader');
      ...
      const group = userGroup || (window.aflUser && window.aflUser.group) || 'all_access';
      const accessible = docs.filter(o => {
        const groups = o.accessGroups;
        if (!groups || !Array.isArray(groups) || groups.length === 0) return true; // no restriction
        return groups.includes(group) || groups.includes('all_access');
      });

      // Map Firestore doc shape → shared.js fund shape
      return accessible.map((o, idx) => {
        const inc = o.projectedIncome || {};
        return {
          id: o.id, // keep string Firestore ID
          _firestoreId: o.id,
          name: o.identity?.offeringName || '',
          sponsor: o.sponsor?.name || '',
          assetClass: o.identity?.assetClass || '',
          sector: o.identity?.sector || '',
          focus: o.identity?.focus || '',
          offeringStructure: o.identity?.offeringStructure || '',
          taxReporting: o.identity?.taxReporting || '',
          status: o.status || 'open',
          filedRaise: o.raise?.filedRaise || null,
          currentRaise: o.raise?.currentRaise || null,
          equityRemaining: o.raise?.remainingRaise || null,
          offeringOpen: o.raise?.offeringOpen || null,
          y1coc: o.terms?.year1CashOnCash || null,
          distFrequency: o.terms?.distributionFrequency || '',
          ltv: o.terms?.loanToValue || null,
          preferred: o.terms?.preferred || null,
          minInvest: o.terms?.minimumDST || null,
          holdPeriod: o.terms?.holdPeriod || null,
          upReit: o.terms?.upreit721 || false,
          promote: o.terms?.promote || '',
          numAssets: o.property?.numAssets || null,
          location: Array.isArray(o.property?.locations) ? o.property.locations.join(', ') : (o.property?.locations || ''),
          buildingAge: o.property?.buildingAge || null,
          occupancy: o.property?.pctLeased || null,
          debtTerms: o.property?.debtTerms || '',
          leaseTerms: o.property?.leaseTerms || '',
          rentEscalations: o.property?.rentEscalations || '',
          avgLeaseTerm: o.property?.avgLeaseTermRemaining || null,
          purchasePrice: o.economics?.purchasePrice || null,
          appraisedValue: o.economics?.appraisedValuation || null,
          loadedPrice: o.economics?.loadedPrice || null,
          capRate: o.economics?.acquisitionCapRate || null,
          repComp: o.economics?.repComp || null,
          salesLoad: o.economics?.salesLoad || null,
          reserve: o.economics?.reserve || null,
          gpCommit: o.economics?.gpCommit || null,
          income: [
            inc.year1, inc.year2, inc.year3, inc.year4, inc.year5,
            inc.year6, inc.year7, inc.year8, inc.year9, inc.year10
          ].map(v => v ?? null),
          brochureUrl: o.documents?.brochureUrl || '',
          ppmUrl: o.documents?.ppmUrl || '',
          trackRecordUrl: o.documents?.trackRecordUrl || '',
          videoUrl: o.documents?.videoUrl || '',
          salesTeamUrl: o.documents?.salesTeamMapUrl || '',
          sponsorNewsUrl: o.documents?.sponsorNewsUrl || '',
          accessGroups: o.accessGroups || [],
          customFields: o.customFields || {},
          displayLabel: (o.identity?.offeringName || '').length > 40
            ? (o.identity?.offeringName || '').slice(0, 38) + '…'
            : (o.identity?.offeringName || 'Offering'),
        };
      });
    } catch (err) {
      console.warn('[AFL] Firestore load failed, will fall back to Sheets:', err);
      return null;
    }
  }

  // ─────────────────────────────────────────────────────────────
  //  loadFunds()  — public API  (Firestore first, Sheets fallback)
  // ─────────────────────────────────────────────────────────────
  ...
  async function loadFunds(forceRefresh = false) {
    ...
    _loadPromise = (async () => {
      try {
        // ── Try Firestore first ──
        const userGroup = window.aflUser?.group || null;
        const firestoreFunds = await loadFundsFromFirestore(userGroup);

        if (firestoreFunds && firestoreFunds.length > 0) {
          console.log(`[AFL] Loaded ${firestoreFunds.length} offerings from Firestore (group: ${userGroup || 'all_access'})`);
          state.funds = firestoreFunds;
          ssSet(LS_KEYS.funds, firestoreFunds);
          return firestoreFunds;
        }

        // ── Fall back to Google Sheets ──
        console.warn('[AFL] Firestore returned no offerings — falling back to Google Sheets');
  ...
  function suitScore(fund, client, peer) {
    ...
    const minInv    = parseMoneyStr(String(fund.minInvest ?? fund.minimum ?? '')) || null;
  ...
  function peerStats(funds) {
    ...
    function toNum(v) {
      ...
      // Parse money strings like "$136.25 M"
    ...
    function avg(field) {
      const vals = arr.map(f => toNum(f[field])).filter(v => v != null);
  ...
  const fmt = {
    ...
    money(v) {
      if (v == null) return '—';
      // If already a formatted string (e.g. "$136.25 M"), pass through
  ...
  console.log(`[AFL] shared.js v${AFL.version} loaded — Firestore-first offering loader — group access filtering`);
