...
(function() {
  ...
  // ─── FRED FETCH (CORS-proxied for GitHub Pages) ───
  ...
  // ─── RENDER COMPACT CARD (for offering detail view) ───
  async function renderCompact(containerId, opts) {
    injectStyles();
    // Add compact-specific styles
    if (!document.getElementById('mi-widget-compact-styles')) {
      const cs = document.createElement('style');
      cs.id = 'mi-widget-compact-styles';
      cs.textContent = `
        ...
        .miw-compact-top::after { content: ''; position: absolute; right: -40px; top: -40px; width: 160px; height: 160px; border-radius: 50%; background: rgba(255,255,255,0.03); }
        ...
        .miw-compact-ring .trk { stroke: rgba(255,255,255,0.15); }
        ...
        .miw-compact-ring .sc-lbl { font-size: 8px; text-transform: uppercase; letter-spacing: 0.8px; color: rgba(255,255,255,0.5); margin-top: 1px; }
        .miw-compact-meta { flex: 1; }
        .miw-compact-meta .cm-title { font-size: 15px; font-weight: 700; color: #fff; margin-bottom: 2px; }
        .miw-compact-meta .cm-sub { font-size: 12px; color: rgba(255,255,255,0.45); margin-bottom: 8px; }
        .miw-compact-tags { display: flex; gap: 5px; flex-wrap: wrap; }
        .miw-compact-tags .ct { font-size: 10px; font-weight: 600; padding: 2px 8px; border-radius: 100px; letter-spacing: 0.3px; }
        .miw-compact-tags .ct.pos { background: rgba(26,122,74,0.25); color: #6EE7A0; }
        .miw-compact-tags .ct.neg { background: rgba(220,38,38,0.25); color: #FCA5A5; }
        .miw-compact-tags .ct.neu { background: rgba(255,255,255,0.12); color: rgba(255,255,255,0.7); }
        ...
        .miw-compact-stat .cs-lbl { font-size: 10px; font-weight: 600; color: #94A3B8; text-transform: uppercase; letter-spacing: 0.5px; margin-top: 2px; }
        ...
        .miw-compact-brief h4 { font-size: 11px; font-weight: 700; color: #1C3A63; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 6px; display: flex; align-items: center; gap: 6px; }
    ...
    // Show loading
    container.innerHTML = `<div class="miw-compact" style="padding:32px;text-align:center"><div class="miw-spin" style="width:28px;height:28px;border:3px solid #E2E8F0;border-top-color:#2A6496;border-radius:50%;animation:spin 0.8s linear infinite;margin:0 auto 10px"></div><div style="font-size:12px;color:#94A3B8">Loading market intelligence…</div></div>`;
    ...
    // Parallel API calls — same as full widget
    ...
    const d = {
      ...
      medianIncome, vacancyRate, capRateSpread
    };
    ...
    if (unempLocal !== null && unempNat !== null) {
      tags.push(unempLocal < unempNat ? '<span class="ct pos">Low Unemployment</span>' : '<span class="ct neg">High Unemployment</span>');
    }
    if (hpiGrowth !== null) {
      tags.push(hpiGrowth > 0 ? '<span class="ct pos">Price Growth</span>' : '<span class="ct neg">Price Decline</span>');
    }
    if (capRateSpread !== null) {
      tags.push(capRateSpread > 150 ? '<span class="ct pos">Wide Spread</span>' : capRateSpread > 50 ? '<span class="ct neu">Adequate Spread</span>' : '<span class="ct neg">Tight Spread</span>');
    }

    // Offering-contextual narrative (not the generic one — this references the offering)
    const offeringName = opts.offeringName || 'This offering';
    const propLabel = propType.charAt(0).toUpperCase() + propType.slice(1);
    const briefParts = [];

    if (treasury10 !== null) {
      const env = treasury10 < 3.5 ? 'favorable' : treasury10 < 4.5 ? 'moderate' : 'challenging';
      briefParts.push(`The current rate environment is ${env} for ${propLabel.toLowerCase()} investments, with the 10-Year Treasury at ${treasury10.toFixed(2)}%.`);
    }
    if (capRateSpread !== null) {
      const q = capRateSpread > 200 ? 'compelling' : capRateSpread > 100 ? 'adequate' : 'compressed';
      briefParts.push(`${offeringName}'s implied cap rate spread of ${capRateSpread}bps over Treasuries is ${q}.`);
    }
    if (unempLocal !== null && unempNat !== null) {
      const vs = unempLocal < unempNat ? 'below' : 'above';
      briefParts.push(`${metro.name}'s ${unempLocal.toFixed(1)}% unemployment rate is ${vs} the ${unempNat.toFixed(1)}% national average, ${unempLocal < unempNat ? 'signaling a healthy local economy' : 'which warrants monitoring'}.`);
    }
    if (hpiGrowth !== null) {
      briefParts.push(`Local home prices have ${hpiGrowth > 0 ? 'grown' : 'declined'} ${Math.abs(hpiGrowth).toFixed(1)}% YoY — ${hpiGrowth > 0 ? 'supporting asset valuations in the market' : 'which may pressure near-term property values'}.`);
    }
    if (creDelinq !== null) {
      briefParts.push(`CRE loan delinquency stands at ${creDelinq.toFixed(2)}%, ${creDelinq < 2 ? 'indicating healthy market conditions' : 'elevated relative to historical norms'}.`);
    }
    const briefText = briefParts.length ? briefParts.join(' ') : 'Market data is being assembled. Full analysis available in the Market Intelligence module.';

    // 4 key stat deltas
    const unempDelta = (unempLocal !== null && unempNat !== null)
      ? `<span class="cs-delta ${unempLocal < unempNat ? 'up' : 'dn'}">${unempLocal < unempNat ? '▼' : '▲'} vs ${unempNat.toFixed(1)}% nat'l</span>` : '';
    const hpiDelta = hpiGrowth !== null
      ? `<span class="cs-delta ${hpiGrowth > 0 ? 'up' : 'dn'}">${hpiGrowth > 0 ? '▲' : '▼'} ${Math.abs(hpiGrowth).toFixed(1)}% YoY</span>` : '';
    ...
    <div class="miw-compact">
      <div class="miw-compact-top">
        ...
        <div class="miw-compact-meta">
          <div class="cm-title">📊 ${metro.name} — ${propLabel}</div>
          <div class="cm-sub">Market Health Score · ${score >= 65 ? 'Above Average' : score >= 45 ? 'Near Average' : 'Below Average'}</div>
          <div class="miw-compact-tags">${tags.join('')}</div>
        </div>
      </div>

      <div class="miw-compact-stats">
        <div class="miw-compact-stat">
          <div class="cs-val">${fP(treasury10)}</div>
          <div class="cs-lbl">10Y Treasury</div>
        </div>
        <div class="miw-compact-stat">
          <div class="cs-val">${fP(unempLocal, 1)}</div>
          <div class="cs-lbl">${metro.name} Unemp.</div>
          ${unempDelta}
        </div>
        <div class="miw-compact-stat">
          <div class="cs-val">${hpiGrowth !== null ? (hpiGrowth > 0 ? '+' : '') + hpiGrowth.toFixed(1) + '%' : '—'}</div>
          <div class="cs-lbl">HPI YoY</div>
          ${hpiDelta}
        </div>
        <div class="miw-compact-stat">
          <div class="cs-val">${fP(creDelinq)}</div>
      ...
      <div class="miw-compact-foot">
        ...
        <a class="miw-compact-link" onclick="if(window.navigateTo)navigateTo('market_intel')">View Full Analysis →</a>
      </div>
    </div>`;

    return { score, narrative: briefText, data: d };
  }

  // ─── RENDER WIDGET (full version) ───
  ...
  // ─── EXPORT ───
  window.MarketIntelWidget = { render, renderCompact, renderFromOffering, resolveCBSA, normalizePropertyType };
