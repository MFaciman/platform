...
(function() {
  ...
  async function expandFull(triggerEl) {
    // Walk up from the clicked <a> to find the miw-compact container div, then its parent
    ...
    // Show loading state while full render runs
    container.innerHTML = `<div style="padding:24px;text-align:center;color:#94A3B8;font-size:13px">
      <div style="width:28px;height:28px;border:3px solid #E2E8F0;border-top-color:#2A6496;border-radius:50%;animation:spin .8s linear infinite;margin:0 auto 12px"></div>
      Loading full analysis…
    </div>`;

    // Run the full render into the container
    await render(container.id, opts);

    // After render, inject a collapse button at the top of the miw panel
    const miwEl = container.querySelector('.miw');
    if (miwEl) {
      const collapseBar = document.createElement('div');
      collapseBar.style.cssText = 'display:flex;justify-content:flex-end;padding:8px 20px 0;';
      collapseBar.innerHTML = `<a onclick="window.MarketIntelWidget.collapseToCompact(this)"
        style="font-size:12px;font-weight:600;color:#2A6496;cursor:pointer;display:flex;align-items:center;gap:4px;text-decoration:none">
        ⤡ Collapse
      </a>`;
      miwEl.insertBefore(collapseBar, miwEl.firstChild);
    }
  }

  async function collapseToCompact(triggerEl) {
    const miwEl = triggerEl.closest('.miw');
    if (!miwEl) return;
    const container = miwEl.parentElement;
