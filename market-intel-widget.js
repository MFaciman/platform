...
(function() {
  ...
  function resolveLocationType(locationStr) {
    ...
    // Check for state-only pattern: all parts are 2-letter state codes,
    ...
    if (rawParts.length && rawParts.every(p => stateOnlyRe.test(p))) {
      // Strip count suffixes, return clean 2-letter codes
      return { type: 'national', states: rawParts.map(p => p.match(/^([A-Z]{2})/)[1]) };
    }

    // Extract "City, ST" pairs (handles both & and , delimiters)
    const cityStateRe = /([^,&]+,\s*[A-Z]{2})/g;
  ...
  async function _renderCompactMulti(container, cities, propType, capRate, offeringName, opts) {
    ...
    const tags = [
      ...
      avgScore >= 65 ? '<span class="ct pos">Strong Combined</span>' : avgScore >= 45 ? '<span class="ct neu">Near Average</span>' : '<span class="ct neg">Mixed Signals</span>',
      primaryD.creDelinq !== null && primaryD.creDelinq < 2 ? '<span class="ct pos">Low Delinquency</span>' : ''
    ].filter(Boolean).join('');
    ...
    <div class="miw-compact">
      <div class="miw-compact-top">
        ...
        <div class="miw-compact-meta">
          <div class="cm-title">📊 Multi-Market · ${_cap(propType)}</div>
      ...
      <div class="miw-compact-stats">
        ...
        <div class="miw-compact-stat">
          <div class="cs-val">${fP(primaryD.unempNat, 1)}</div>
          <div class="cs-lbl">Nat'l Unemp.</div>
        </div>
        <div class="miw-compact-stat">
          <div class="cs-val">${fP(primaryD.mortgage30)}</div>
