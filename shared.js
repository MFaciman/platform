...
(function (global) {
  ...
  // Percent fields — GViz returns these as decimals (e.g. 0.0525 for 5.25%)
  const PCT_FIELDS = new Set([
    'y1coc','ltv','preferred','occupancy','capRate','repComp','salesLoad',
   ...
   *
   * KEY FIX: GViz returns percentage-formatted cells as raw decimals
   * in cell.v (e.g. 0.0525 for 5.25%) with the formatted string "5.25%"
   * in cell.f. We detect this and multiply by 100 so all percent fields
   * are stored as human-readable percentages (5.25, not 0.0525).
   */
      ...
      .map((row, rowIdx) => {
        ...
        cols.forEach((colLabel, ci) => {
          ...
          if (NUM_FIELDS.has(field) && val !== null && val !== '') {
            ...
            const n = parseFloat(rawStr.replace(/[,$%\s]/g, ''));

            if (isFinite(n)) {
              if (PCT_FIELDS.has(field)) {
                if (hasPercentSign) {
                  // String like "5.25%" — already human-readable, store as-is
                  val = n;
                } else if (typeof cell.v === 'number' && Math.abs(n) < 1) {
                  // GViz numeric decimal (0.0525) — multiply by 100
                  val = n * 100;
                } else {
                  // Already a whole-number percent (e.g. 29 for 29%) — store as-is
                  val = n;
                }
              } else {
                val = n;
  ...
  function suitScore(fund, client) {
    // Gate: client name only — works with partial profiles
    if (!client || !client.name) return null;

    let earned = 0, possible = 0;
    const sp = client._shell && client._shell.structurePreference;

    // ── LTV risk (weight 20) — fund-only, no client data needed ──
    const ltv = fund.ltv;
    if (ltv != null && isFinite(ltv)) {
      possible += 20;
      if (ltv <= 60)      earned += 20;
      else if (ltv <= 65) earned += 14;
      else if (ltv <= 75) earned += 6;
      // No-debt penalty from shell structure preferences
      if (sp && sp.noDebt && ltv > 0) earned -= 10;
    }

    // ── Min investment vs exchange equity (weight 15) ──
    const min = fund.minInvest != null ? fund.minInvest : fund.minimum;
    if (min != null && isFinite(min) && client.exchangeAmount != null && client.exchangeAmount > 0) {
      possible += 15;
      if (min <= client.exchangeAmount)            earned += 15;
      else if (min <= client.exchangeAmount * 1.25) earned += 5;
    }

    // ── Cash-on-cash yield (weight 15) — fund-only ──
    const _coc = fund.y1coc != null ? fund.y1coc : fund.y1CoC;
    if (_coc != null && isFinite(_coc)) {
      possible += 15;
      if (_coc >= 5.5)      earned += 15;
      else if (_coc >= 4.0) earned += 10;
      else if (_coc >= 3.0) earned += 5;
    }

    // ── Property type / sector match (weight 10) ──
    if (Array.isArray(client.propTypes) && client.propTypes.length > 0 && (fund.sector || fund.propType || fund.assetClass)) {
      possible += 10;
      if (client.propTypes.includes(fund.sector) || client.propTypes.includes(fund.propType) || client.propTypes.includes(fund.assetClass)) {
        earned += 10;
      }
    }

    // ── Occupancy (weight 10) — fund-only ──
    const _occ = fund.occupancy != null ? fund.occupancy : fund.pctLeased;
    if (_occ != null && isFinite(_occ)) {
      possible += 10;
      if (_occ >= 95)      earned += 10;
      else if (_occ >= 90) earned += 7;
      else if (_occ >= 80) earned += 3;
    }

    // ── Hold period vs age (weight 10) ──
    const age = client.age ? parseFloat(client.age) : NaN;
    const hp  = fund.holdPeriod ? parseFloat(fund.holdPeriod) : NaN;
    if (!isNaN(age) && !isNaN(hp)) {
      possible += 10;
      if (age + hp <= 78)      earned += 10;
      else if (age + hp <= 83) earned += 6;
      else if (age + hp <= 88) earned += 2;
    }

    // ── Structure preference: DST-only (weight 10) ──
    if (sp && sp.dstOnly) {
      possible += 10;
      const struct = (fund.offeringStructure || fund.structure || '').toLowerCase();
      if (struct.includes('dst')) earned += 10;
    }

    // ── 721 UpREIT preference (weight 5) ──
    if (sp && sp.upreit) {
      possible += 5;
      if (fund.upReit) earned += 5;
    }

    // No factors available — neutral score
    if (possible === 0) return { score: 50, tooltip: 'No scoring factors available' };
    const raw = Math.min(100, Math.max(0, Math.round((earned / possible) * 100)));
    return { score: raw, tooltip: '' };
  }
  ...
  const AFL = {
    ...
    extractStates,
    parseGvizDate,
    version: '1.1.0',  // bumped for percent fix
  };
